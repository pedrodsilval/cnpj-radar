...
export class CnpjController {
  ...
  @Post('lote')
  async lote(@Body() body: { cnpjs?: unknown }) {
    ...
    return this.cnpjService.consultarLote(body.cnpjs as string[]);
  }

  // Rota mais especifica declarada antes da rota parametrica generica
  @Get(':cnpj/pdf')
  async pdf(@Param('cnpj') cnpj: string, @Res() res: Response) {
    const resultado = await this.cnpjService.consultarCnpj(cnpj);
