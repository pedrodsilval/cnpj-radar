...
>
> **Confirmado com um segundo CNPJ limpo (`41530027000173`, fornecido pelo usuário, com 3 certidões "Negativa" recentes):** repeti o mesmo teste manual — `Emitir Nova Certidão` → **"A certidão foi emitida com sucesso"**, na hora, sem fricção. Isso prova que o hCaptcha invisível resolve normalmente num navegador real e que o site emite certidão automaticamente sem problema quando o CNPJ está de fato elegível. **Fecha a investigação:** não há bloqueio técnico nenhum no lado da Receita Federal — a única pendência real é atualizar o scraper pra usar o fluxo atual (`Emissao/verificar` + `Emissao`) em vez do `validar-contribuinte` desatualizado.
>
