...
| **Certidão Municipal** | ❌ **Nenhuma** | `consultarCertidaoMunicipal()` **nunca abre um browser** — é 100% um stub que devolve link manual, inclusive para Salvador (a própria cidade da Everest/FK's). O mapa `portaisMunicipais` tem só 5 entradas (SP, RJ, BH, RS, PR) e duas delas (RS, PR) apontam erroneamente para portais **estaduais** de fazenda, não municipais. |
| **Inscrição Municipal (IM/CCM)** | ❌ **Nenhuma** | `consultarInscricaoMunicipal()` — mesmo caso: 100% stub, nenhuma automação real pra nenhum município, nem Salvador. |
