...
### 2.2 CND Federal / CNDT Trabalhista / Dívida Ativa da União — chave 2captcha ok, hCaptcha não resolve
...
- **Verificado direto no painel do 2captcha (21/08/2026):** no dia do teste, a conta processou **5 captchas resolvidos e 3 não resolvidos** (aba Costs, `/statistics/graph_captures`) — ou seja, o 2captcha consegue resolver a maioria dos hCaptchas na prática. Isso sugere que a falha final do CND Federal pode não ser só "hCaptcha não resolvido pelo 2captcha" — é plausível que, em pelo menos algumas tentativas, o token **é resolvido corretamente mas rejeitado pela própria Receita Federal** na validação (`CaptchaFalhaValidacao`, já tratado no código como retry), comportamento típico de hCaptcha "invisible"/enterprise com análise de risco adicional. O código atual não distingue "2captcha não resolveu" de "resolveu mas o site rejeitou" na mensagem final — ficaria mais preciso separar os dois casos, mas isso é trabalho novo em cima de e-CAC, então **não implementado agora** por causa da decisão de pausa (ver abaixo).
