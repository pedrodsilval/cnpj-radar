...
>
> `CAPTCHA_API_URL`/`CAPTCHA_API_KEY` configuradas no serviço `cnpj-radar` do Render apontando pra `https://api-captcha-xxm9.onrender.com` — confirmado `/health` respondendo OK e o cnpj-radar redeployado e subindo limpo (`Nest application successfully started`) com as novas envs. **Ainda não testado end-to-end** (uma consulta real de CND Federal/CNDT batendo na api_captcha e caindo no 2captcha só se ela falhar) — próximo passo natural quando for reconfirmar o estado das certidões.
