...
>
> **Estado atual, honesto:** a integração cnpj-radar ↔ api_captcha está funcionando (chega, autentica, tenta, registra o uso) e falha *de forma limpa* em vez de crashar — mas ainda não resolve nenhum hCaptcha de verdade, porque **`TWOCAPTCHA_API_KEY` nunca foi configurada no serviço `api-captcha` do Render**. Sem ela, toda tentativa de CND Federal/Dívida Ativa continua caindo pro 2captcha *do próprio cnpj-radar* (que já tinha essa chave, guardada criptografada em `credenciais` — não recuperável pela UI/banco por design: "Credenciais são sensíveis. Valores nunca são exibidos após o cadastro."). **Decisão pendente do usuário:** ou (a) cadastrar a mesma chave de 2captcha como `TWOCAPTCHA_API_KEY` no Render do serviço `api-captcha` (elimina a chamada dupla — hoje o 2captcha é acionado duas vezes: uma vez, sem sucesso, dentro da api_captcha, e de novo pelo fallback do próprio cnpj-radar), ou (b) aceitar deixar assim — funcionalmente equivalente pro usuário final, só um pouco mais lento (~20-90s de latência extra por tentativa de hCaptcha, sem custo adicional). Para CNDT (tipo `image`, usa OCR local via easyocr antes de cair pro pago) o ganho potencial é maior e não depende dessa decisão — vale reconfirmar numa consulta futura se o OCR local já está resolvendo algum captcha sozinho.
